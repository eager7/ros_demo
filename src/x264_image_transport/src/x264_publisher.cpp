...
x264Publisher::x264Publisher()
    ...
    buffer_(NULL),
	  sws_ctx_(NULL),
	  initialized_(false),
    qmax_(51), bitrate(512000), gfps(30) {
...
void x264Publisher::configCb(Config& config, uint32_t level) {
  ...
  ROS_WARN("Configuration changed bitrate: %i",config.bitrate);
...
void x264Publisher::initialize_codec(int width, int height, int fps, const std::string& encoding) const {
  ...
  encCdcCtx_->bit_rate = bitrate; //Seems a good starting point
  ...
  initialized_ = true;
	ROS_INFO("x264Publisher::initialize_codec(): codec initialized (width: %i, height: %i, fps: %i, bitrate %i)",width,height,fps, encCdcCtx_->bit_rate);
  pthread_mutex_unlock (&mutex_);
}
